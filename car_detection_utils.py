...
nodejs_CarStatistics_url = 'http://10.10.53.205:3001/getCarStatistics'
CarStatistics_dict = {}
...
def stop_car_recognition(frame, display_frame, tracker, stop_car, car_flow, point_polylines, stop_flag, track_no_path,
    ...
    track = tracker
    tracker_x = float(track[0] / frame.shape[0])
    tracker_y = float(track[1] / frame.shape[1])
    tracker_w = float(track[2] / frame.shape[0])
    tracker_h = float(track[3] / frame.shape[1])

    xmin = int((tracker_x - tracker_w / 2) * frame.shape[1])
    xmax = int((tracker_x + tracker_w / 2) * frame.shape[1])
    ymin = int((tracker_y - tracker_h / 2) * frame.shape[0])
    ymax = int((tracker_y + tracker_h / 2) * frame.shape[0])
    xstop = (xmin + xmax) / 2
    ystop = ymax

    list_xpoint = [point_polylines[0][0][0],point_polylines[1][0][0],point_polylines[2][0][0],point_polylines[3][0][0]]
    list_ypoint = [point_polylines[0][0][1],point_polylines[1][0][1],point_polylines[2][0][1],point_polylines[3][0][1]]
    ...
    if track[4] in car_flow:
        display_frame = puttext_in_chinese(display_frame, str(car_flow[track[4]][0]), (xstop, ymax), (255, 0, 255), fontsize=60)
